...
const BattleResults = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${darkMode ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <div className="flex-1 pt-12 px-4 pb-12">
      <div className="max-w-4xl mx-auto text-center mb-12">
          <h1 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-indigo-600 mb-4 tracking-tight">
              Battle Complete
          </h1>
          <p className={`text-lg font-bold ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Room: <span className="font-mono text-indigo-500">{matchId}</span></p>
      </div>

      <div className="max-w-3xl mx-auto flex flex-col md:flex-row items-end justify-center gap-4 mb-16 h-80">
        {/* Render podium top 3 */}
      ...
      <div className="max-w-2xl mx-auto flex justify-center gap-4">
          <button onClick={() => navigate('/battle-lobby')} className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold transition shadow-lg shadow-indigo-600/30">
              Play Again
          </button>
          <button onClick={() => navigate('/profile')} className={`px-8 py-3 rounded-xl font-bold transition ${darkMode ? 'bg-slate-700 hover:bg-slate-600 text-white' : 'bg-slate-200 hover:bg-slate-300 text-slate-800'}`}>
              Back to Profile
          </button>
      </div>
      </div>
